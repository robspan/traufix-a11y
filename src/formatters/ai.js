 ...
 *
 * Simple backlog for AI assistants to work through.
 * Grouped by file for quick identification.
 * Fix any issue, re-run to see what's left.
 */
...
/**
 * Format results as simple backlog
 */
function format(results, options = {}) {
  ...
  // Calculate issue points for each component (sum of weights × usage)
  for (const [_, data] of issuesByComponent) {
    let basePoints = 0;
    for (const issue of data.issues) {
      basePoints += getCheckWeight(issue.check);
    }
    const usageCount = data.affectedUrls.size || 1;
    data.issuePoints = {
      basePoints,
      usageCount,
      totalPoints: basePoints * usageCount
    };
  }

  // Sort components by total issue points (highest priority first)
  const sortedComponents = [...issuesByComponent.entries()]
    .filter(([_, data]) => data.issues.length > 0)
    .sort((a, b) => b[1].issuePoints.totalPoints - a[1].issuePoints.totalPoints);
  ...
  const totalPoints = sortedComponents.reduce((sum, [_, data]) => sum + data.issuePoints.totalPoints, 0);
  lines.push(`# GENERATED by mat-a11y - do not edit`);
  lines.push(`# Fix any issue, re-run: npx mat-a11y --headless`);
  lines.push(`# traufix.de | freelancermap.de/profil/robin-spanier`);
  lines.push('');
  lines.push(`${totalIssues} issues | ${totalPoints} priority points | ${sortedComponents.length} files`);
  lines.push(`# Components sorted by priority (fix top items first)`);
  lines.push('');
  ...
  for (const [componentName, data] of sortedComponents) {
    ...
    const points = data.issuePoints;
    const pointsDisplay = points.usageCount > 1
      ? `${points.totalPoints}pts (${points.basePoints}×${points.usageCount} uses)`
      : `${points.basePoints}pts`;

    lines.push(`════════════════════════════════════════`);
    lines.push(`COMPONENT: ${componentName} [${pointsDisplay}]`);
    ...
    for (const issue of data.issues) {
      if (!issuesByCheck.has(issue.check)) {
        issuesByCheck.set(issue.check, { fix: issue.fix, elements: [], weight: getCheckWeight(issue.check) });
      }
      if (issue.element) {
    ...
    // Sort checks by weight (highest priority first)
    const sortedChecks = [...issuesByCheck.entries()].sort((a, b) => b[1].weight - a[1].weight);

    for (const [check, info] of sortedChecks) {
      ...
      lines.push(`[w${info.weight}] ${check}${elementStr}${countStr}`);
      if (info.fix) {
        lines.push(`  → ${info.fix}`);
      }
    }
...
function formatComponentResults(results, lines) {
  ...
  lines.push(`# Fix any issue, re-run: npx mat-a11y --headless`);
  lines.push('');
  lines.push(`${totalIssues} issues in ${withIssues.length} components`);
  lines.push('');
...
module.exports = {
  name: 'ai',
  description: 'Accessibility backlog for AI/LLM to fix issues',
  category: 'docs',
  output: 'text',
  fileExtension: '.backlog.txt',
  mimeType: 'text/plain',
  format
};
